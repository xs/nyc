...
function generateSummary() {
  ...
  console.log(
    `📁 Sample files created: ${sampleFiles.length} (in data/sample/)`
  );
  ...
  for (const originalFile of originalFiles) {
    ...
    if (fs.existsSync(samplePath)) {
      ...
      const reduction = (
        ((originalStats.size - sampleStats.size) / originalStats.size) *
        100
      ).toFixed(1);
      ...
      const buildingCount = (
        sampleContent.match(/<bldg:Building gml:id=/g) || []
      ).length;
      ...
      console.log(
        `DA${daNumber.padStart(2)} | ${originalSizeMB.padStart(11)} MB | ${sampleSizeMB.padStart(10)} MB | ${reduction.padStart(8)}% | ✅ Complete`
      );
    } else {
      console.log(
        `DA${daNumber.padStart(2)} | ${originalSizeMB.padStart(11)} MB | ${'N/A'.padStart(10)} | ${'N/A'.padStart(8)} | ❌ Pending`
      );
    }
  }
  ...
  const overallReduction = (
    ((totalOriginalSize - totalSampleSize) / totalOriginalSize) *
    100
  ).toFixed(1);

  console.log('\n📊 Overall Summary:');
  console.log(
    `Files processed: ${sampleFiles.length}/${originalFiles.length} (${((sampleFiles.length / originalFiles.length) * 100).toFixed(1)}%)`
  );
  console.log(
  ...
  console.log(
    `Sample buildings created: ${totalSampleBuildings.toLocaleString()}`
  );
