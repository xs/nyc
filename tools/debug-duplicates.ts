...
function fixBuildingIds(buildings: Building[]): Building[] {
  ...
  console.log(
    `Removed duplicates: ${buildings.length - fixedBuildings.length}`
  );
...
function main() {
  ...
  if (args.length === 0) {
    console.log(
      'Usage: npx ts-node tools/debug-duplicates.ts <path-to-footprints.geojson>'
    );
    console.log(
      'Example: npx ts-node tools/debug-duplicates.ts out/williamsburg/footprints.geojson'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (
      ...
      analysis.duplicateFootprints.length > 0
    ) {
      console.log(`\n=== Applying Fixes ===`);
