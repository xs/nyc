...
const IN_DIR = getStringArg('in', ['i'], {
  default: './data/downtown',
}) as string;
const OUT_DIR = getStringArg('out', ['o'], {
...
const CENTER_LAT: number = 40.71671893970987;
const CENTER_LNG: number = -73.96201793555863;

// Transform center point to EPSG:3857
const centerMercator: number[] = proj4('EPSG:4326', 'EPSG:3857').forward([
  CENTER_LNG,
  CENTER_LAT,
]);

const toMerc: Converter = proj4('EPSG:2263', 'EPSG:3857');
const FT_TO_M = 0.3048;

// Modified reprojection function that centers coordinates around the specified point
const reprojXY = (x: number, y: number): [number, number] => {
...
const nsLocal = (tag: string): string =>
  tag?.split(':').pop()?.split('}').pop() || '';

function* walk(node: GMLNode): Generator<GMLNode> {
...
function posListToXYZ(text: string): Vec3[] {
...
function planeBasis3D(points: Vec3[]): planeBasis {
  const p0 = points[0];
  let v1: Vec3 = [1, 0, 0];
  ...
  return {
    ...
    n: nrm,
    to2D: (p: Vec3): Vec2 => {
      const w: Vec3 = [p[0] - p0[0], p[1] - p0[1], p[2] - p0[2]];
...
function triangulate3D(rings3D: Ring3D[]): Mesh {
  const basis = planeBasis3D(rings3D[0]);
  const rings2D: Ring2D[] = rings3D.map((r) => r.map(basis.to2D));
  const flat: number[] = [];
  const holes: number[] = [];
  let offset = 0;
  for (let i = 0; i < rings2D.length; i++) {
    const r: Vec2[] = rings2D[i];
    if (i > 0) {
  ...
  const idx: number[] = earcut(flat, holes.length ? holes : undefined, 2);
  const verts3D: Vec3[] = rings2D.flat().map(basis.to3D);

  // TODO: figure out typing here. also what does this do, even
  const positions = [];
  const remap = new Map();
...
function extrudeFootprint(footprint: Vec2[], height: number): Mesh {
...
type GMLNode = { [key: string]: any };
/* =========================
   CityGML readers
   ========================= */
function containsTag(node: GMLNode, name: string): boolean {
  for (const k of Object.keys(node)) {
...
function processGMLSync(filePath: string, opts: { lod2: boolean }): Building[] {
  ...
  for (const node of walk(root)) {
    ...
    const buildingId =
      currentBuildingId ||
      node['gml:id'] ||
      `b_${buildingGroups.size}_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`;
  ...
  for (const [buildingId, elements] of buildingGroups) {
    ...
    if (lod2) {
      ...
      for (const element of elements) {
        for (const rings of element.polys) {
          const rings3D: Ring3D[] = rings.map((r) =>
            r.map(([x, y, z]) => {
...
/* =========================
   Outputs
   ========================= */
function writeFootprintsGeoJSON(buildings: Building[], outPath: string): void {
...
function writeFlatbushIndex(
  ...
  const data = idx.data as Uint8Array;
